...
public class HystrixDashboardProxyEurekaTest {
	...
	private static final transient Logger log = LoggerFactory.getLogger(HystrixDashboardProxyEurekaTest.class);
	...
	public static void setUp(TestContext testContext) throws Exception {
		...
		deployFakeEurekaVerticle().compose(i -> deployDashboard()).compose(i -> initializeHttpClientForDashboard()).setHandler(testContext.asyncAssertSuccess());
	}
	...
	private static Future<Void> initializeHttpClientForDashboard() {
		httpClient = vertx.createHttpClient(new HttpClientOptions().setDefaultHost("localhost").setDefaultPort(7979));
		...
		return new AbstractVerticle() {
			...
			public void start(Future<Void> startFuture) throws Exception {
				...
				router.get("/eureka/v2/apps").handler(context -> {
					...
					vertx.fileSystem().readFile("eureka-data.xml", res -> {
						if (res.succeeded()) {
							response.putHeader(HttpHeaders.CONTENT_TYPE, "application/xml").end(res.result());
						} else {
							response.setStatusCode(500).end("Error while reading eureka data from classpath");
						}
					});
				});

				vertx.createHttpServer(new HttpServerOptions()).requestHandler(router::accept).listen(FAKE_EUREKA_SERVER_PORT, res -> {
					if (res.succeeded()) {
