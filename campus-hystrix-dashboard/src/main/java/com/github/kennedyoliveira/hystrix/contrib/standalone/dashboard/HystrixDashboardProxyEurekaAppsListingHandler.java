...
public class HystrixDashboardProxyEurekaAppsListingHandler implements Handler<RoutingContext> {
	...
	private static final transient Logger log = LoggerFactory.getLogger(HystrixDashboardProxyEurekaAppsListingHandler.class);

	private final HttpClient httpClient;

	HystrixDashboardProxyEurekaAppsListingHandler(Vertx vertx) {
		final HttpClientOptions httpClientOptions = new HttpClientOptions().setConnectTimeout(10000).setTryUseCompression(true).setIdleTimeout(60).setKeepAlive(true)
				.setMaxPoolSize(10);
	...
	public void handle(RoutingContext routingContext) {
		...
		if (isNullOrEmpty(eurekaUrl)) {
			...
			try {
				...
				httpClient.get(port, host, path).handler(clientResp -> clientResp.bodyHandler(respBuffer -> {
					// proxied eureka request OK
					if (clientResp.statusCode() == 200) {
						response.putHeader(HttpHeaders.CONTENT_TYPE, clientResp.headers().get(HttpHeaders.CONTENT_TYPE)).end(respBuffer);
					} else {
						log.error("Fetching eureka apps from url: [{}]\nResponse Status: [{}], Response: [{}]", eurekaUrl, clientResp.statusCode(),
